...
class Xn_from_signal(ABC):
    def __init__(self,nldb,X_order=4,T_urange=[-1, -1],l_out_current=False): # note I need to add user opportunity to set time-range  
    ...
    def solve_lin_system(self,mat,samp,solver):
        mat_dim = int(mat.shape[1])
        out=np.zeros(mat_dim,dtype=np.cdouble)
        SOLVE_MODES = ['stnd', 'lstsq', 'svd']
        if solver not in SOLVE_MODES:
            raise ValueError("Invalid inversion mode. Expected one of: %s" % SOLVE_MODES)
        if solver=="stdn" and ((not IsSquare(mat)) or (not IsWellDefined(mat))):
            solver = "lstsq"
        if solver=="stnd":
    ...
    def perform_analysis(self,solver="stnd"):
        out = np.zeros((self.X_order + 1, self.n_runs, 3), dtype=np.cdouble)
        for i_f in tqdm(range(self.n_runs)):
            for i_d in range(3):
                samp_time, samp_sig= self.get_sampling(i_d,i_f)
                matrix = self.define_matrix(samp_time,i_f)
                raw = self.solve_lin_system(matrix,samp_sig,solver)
